import { createDefaultRpcTransport, createJsonRpcApi, createRpc, type Rpc } from '@solana/rpc';
 ...
 */
export function createSurfnetCheatcodesRpc(endpoint: string = DEFAULT_SURFNET_ENDPOINT): Rpc<SurfnetCheatcodesApi> {
    const api = createJsonRpcApi<SurfnetCheatcodesApi>({
        ...
        responseTransformer: (response: RpcResponse) => {
            ...
            if ('error' in r) {
                throw new Error(
                    `Surfnet RPC error ${r.error.code}: ${r.error.message}${r.error.data ? ` — ${JSON.stringify(r.error.data)}` : ''}`,
                );
            }
